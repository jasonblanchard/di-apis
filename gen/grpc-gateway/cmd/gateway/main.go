...
import (
	...
	gw "gateway/gen/github.com/jasonblanchard/di-apis/gen/pb-go/notebook/v2"
)
...
func SetupMux(ctx context.Context, cfg proxyConfig) *http.ServeMux {
	...
	err := gw.RegisterNotebookHandlerFromEndpoint(ctx, gwmux, cfg.backend, opts)
	if err != nil {
...
func SetupViper() *viper.Viper {
	...
	v := viper.GetViper()
	bindEnvVars(v, strings.NewReplacer(EnvVarSepChar, ViperKeySepChar), strings.ToUpper("Notebook"), ResponseHeaders)
	return v
}
